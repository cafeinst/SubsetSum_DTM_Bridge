...
fixes kk :: nat
  ...
  fixes Lval_at :: "int list ⇒ int ⇒ (nat ⇒ bool) ⇒ nat ⇒ int"
  defines Lval_at_def:
    "Lval_at as s oL j ≡
       from_bits (map oL
  ...
  fixes Rval_at :: "int list ⇒ int ⇒ (nat ⇒ bool) ⇒ nat ⇒ int"
  defines Rval_at_def:
    "Rval_at as s oR j ≡
       from_bits (map oR
  ...
  fixes good :: "int list ⇒ int ⇒ (nat ⇒ bool) ⇒ (nat ⇒ bool) ⇒ bool"
  defines good_def:
    "good as s oL oR ≡
...
definition x0 :: "int list ⇒ int ⇒ bool list" where
  "x0 as s = enc as s kk"
...
proof -
  ...
  have v_in: "enumL as s kk ! j ∈ set (enumL as s kk)"
    using jL by (rule nth_mem)

  have len_bits[simp]:
    "length (to_bits (W as s) (enumL as s kk ! j)) = W as s"
    using to_bits_len_on_enumL v_in by simp

  have slice_x0:
    "map ((!) (x0 as s)) [?a ..< ?a + ?w]
     = to_bits (W as s) (enumL as s kk ! j)"
  ...
  next
    ...
    proof -
      have "j * ?w + t < Suc j * ?w" using tw by simp
      moreover have "Suc j * ?w ≤ length (enumL as s kk) * ?w"
        using jL by (intro mult_right_mono) simp_all
      ultimately show ?thesis
        by (simp add: offL_def length_padL)
    qed
    ...
    proof -
      ...
      then show ?thesis
        using less_padL
        by (simp add: padL_def offL_def nth_append)
    qed
    ...
    have "map ((!) (x0 as s)) [?a ..< ?a + ?w] ! t
      = (x0 as s) ! (?a + t)"
    using nth_map_at idx by simp
    also have "... =
    concat (map (to_bits ?w) (enumL as s kk)) ! (j * ?w + t)"
      using step_enc_pad by simp
    also have "... =
      to_bits ?w (enumL as s kk ! j) ! t"
      using nth_concat_map_fixed[OF fixed_len_meta jL tw] by simp
    finally show "map ((!) (x0 as s)) [?a ..< ?a + ?w] ! t
              = to_bits (W as s) (enumL as s kk ! j) ! t"
      by simp
  qed

  have round:
    "from_bits (to_bits (W as s) (enumL as s kk ! j)) = enumL as s kk ! j"
    using bits_roundtrip v_in by blast

  have slice_enc:
    "map ((!) (enc as s kk)) [?a ..< ?a + ?w]
    = to_bits (W as s) (enumL as s kk ! j)"
    using slice_x0 by simp

  show ?thesis
    unfolding Lval_at_def
    by (simp add: slice_enc round)
qed

lemma Rval_at_on_enc_block:
  assumes jR: "j < length (enumR as s kk)"
  shows   "Rval_at as s ((!) (x0 as s)) j = enumR as s kk ! j"
proof -
  let ?a = "length (enc0 as s) + offR as s kk j"
  let ?w = "W as s"

  have blk_eq: "blockR_abs enc0 as s kk j = {?a ..< ?a + ?w}"
    by (simp add: blockR_abs_def offR_def)  (* adjust if your defs differ *)

  (* fixed width for R-side values *)
  have fixed_len_meta:
    "⋀x'. x' ∈ set (enumR as s kk) ⟹ length (to_bits (W as s) x') = W as s"
    by (simp add: to_bits_len_on_enumR)     (* add this lemma if you don’t already *)

  (* one-step index arithmetic: get into the padR chunk and then into concat(map … enumR) *)
  have step_enc_padR:
    "(x0 as s) ! (length (enc0 as s) + offR as s kk j + t)
     = concat (map (to_bits (W as s)) (enumR as s kk)) ! (j * W as s + t)"
  proof -
    have "(x0 as s) ! (length (enc0 as s) + offR as s kk j + t)
        = (enc0 as s @ padL as s kk @ padR as s kk) !
            (length (enc0 as s) + offR as s kk j + t)"
      by (simp add: enc_def)
    also have "... = (padL as s kk @ padR as s kk) ! (offR as s kk j + t)"
      by (simp add: add.assoc)
    (* offR already includes the padL offset; this picks the right half *)
    also have "... = padR as s kk ! (j * W as s + t)"
      using offR_def length_padL add_mult_distrib2 by (simp add: nth_append)
    also have "... = concat (map (to_bits (W as s)) (enumR as s kk)) ! (j * W as s + t)"
      by (simp add: padR_def)
    finally show ?thesis .
  qed

  (* show the slice equals the bit pattern for enumR!j *)
  have slice_x0_R:
    "map ((!) (x0 as s)) [?a ..< ?a + ?w]
  ...
  proof (rule nth_equalityI)
    ...
    have [simp]: "length (to_bits (W as s) (enumR as s kk ! j)) = W as s"
      using fixed_len_meta v_inR by simp
    show "length (map ((!) (x0 as s)) [?a ..< ?a + ?w])
          = length (to_bits (W as s) (enumR as s kk ! j))" by simp
  next
    fix t assume tlt: "t < length (map ((!) (x0 as s)) [?a ..< ?a + ?w])"
    hence tw: "t < ?w" by simp
    have idx: "[?a ..< ?a + ?w] ! t = ?a + t" using tw by simp
    have "map ((!) (x0 as s)) [?a ..< ?a + ?w] ! t = (x0 as s) ! (?a + t)"
      using idx nth_map by (simp add: tw)
    also have "... = (enc as s kk) ! (?a + t)"
      using x0_is_enc by simp
    also have "... =
      concat (map (to_bits (W as s)) (enumR as s kk)) ! (j * W as s + t)"
      by (metis add.assoc enc_def length_padL nth_append_length_plus 
          offR_def padR_def)
    also have "... = to_bits (W as s) (enumR as s kk ! j) ! t"
      using jR tw nth_concat_map_fixed fixed_len_meta
      by blast
    finally show
      "map ((!) (x0 as s)) [?a ..< ?a + ?w] ! t
  ...
  (* convert to enc-variant if simp already rewrote x0 to enc *)
  have slice_enc_R:
    "map ((!) (enc as s kk)) [?a ..< ?a + ?w]
     = to_bits (W as s) (enumR as s kk ! j)"
    using slice_x0_R by simp

  (* round-trip: from_bits (to_bits W v) = v for v ∈ enumR *)
  have v_inR: "enumR as s kk ! j ∈ set (enumR as s kk)"
    using jR in_set_conv_nth by metis
  have roundR:
    "from_bits (to_bits (W as s) (enumR as s kk ! j)) = enumR as s kk ! j"
    using bits_roundtrip v_inR by blast   (* or your named roundtrip lemma *)
  show ?thesis
    unfolding Rval_at_def
    by (simp add: slice_enc_R roundR)
qed
...
proof -
  ...
  have map_eq:
    ...
    using Rval_at_on_enc_block by auto
...
proof -
  ...
  have R_catalog:
    "set (map (Rval_at as s ((!) (x0 as s))) [0..<length (enumR as s kk)])
     = set (enumR as s kk)"
    using R_catalog_for_enc x0_is_enc by presburger
...
proof
  ...
  have "Rval_at as s oR jR = enumL as s kk ! jL"
    using Eq Lval_at_on_enc_block jL by simp
      ...
      proof -
        ...
        next
          ...
          also have "... ∉ set (enumR as s kk)"
            using False True baseline_only_j j'lt by auto
          finally show ?thesis .
        qed
      qed
