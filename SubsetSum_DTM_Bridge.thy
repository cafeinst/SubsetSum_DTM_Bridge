...
lemma wf_run_of_seen:
  assumes "seenL_run oL oR U \<subseteq> L" and "seenR_run oL oR U \<subseteq> R"
...
lemma coverage_for_enc_blocks_L:
  ...
  shows "\<forall>j<length (enumL as s kk). touches (enc as s kk) (blockL_abs enc0 as s j)"
proof (intro allI impI)
  ...
  proof (rule ccontr)
    assume "\<not> touches ?x (blockL_abs enc0 as s j)"
    then have not_touch: "Base.read0 M ?x \<inter> blockL_abs enc0 as s j = {}"
    ...
    (* outside/inside equalities, unread_agreement, run_eq, etc\<dots> *)
    ...
    proof -
      ...
      next
        case False
        hence ai: "a \<le> i" by simp
        from i_not_blk blk have "\<not> (a \<le> i \<and> i < a + w)"
          by simp
        with ai have "\<not> i < a + w" by blast
        hence ge: "a + w \<le> i" by simp
        show ?thesis
    ...
    (* run/Good mismatch \<rightarrow> contradiction *)
    have run_x:
    ...
    have acc_x:  "run (\<lambda>i. ?x ! i) (\<lambda>i. ?x ! i) (T0 as s) = accepts M ?x"
      using run_diff run_y_eq_run_x by blast
    have acc_y:  "run (\<lambda>i. y ! i) (\<lambda>i. ?x ! i) (T0 as s) = accepts M y"
      using run_diff run_y_eq_run_x by blast
...
proof
  ...
  have "Rval_at as s oR jR \<in> set (enumL as s kk)"
...
lemma blockR_pairwise_disjoint:
  assumes jR: "j < length (enumR as s kk)" and j'R: "j' < length (enumR as s kk)" and ne: "j \<noteq> j'"
  ...
  proof (rule ccontr)
    ...
    proof cases
      ...
      proof -
        ...
        also have "... = oR' i" using ai by (simp add: ilt)
      ...
      proof (rule nth_equalityI)
        show "length (map ((!) y) [a..<a + w]) = length (map oR' [a..<a + w])"
          by simp
      next
      ...
      proof -
        have "Rval_at as s ((!) y) j = from_bits (map ((!) y) [a..<a + w])"
          by (simp add: Rval_at_def a_def w_def)
        also have "... = from_bits (map oR' [a..<a + w])"
          by (simp add: slice_eq_j)
        also have "... = from_bits pat"
          by (simp add: slice_pat)
        also have "... = v_out" by (simp add: pat_val)
        finally show ?thesis .
      qed
      ...
      proof -
        ...
        have agree_on_j':
          "\<And>i. i \<in> blockR_abs enc0 as s kk j' \<Longrightarrow> y ! i = ?x ! i"
        proof -
          fix i assume i_in: "i \<in> blockR_abs enc0 as s kk j'"
          have disj0:
            "blockR_abs enc0 as s kk j' \<inter> blockR_abs enc0 as s kk j = {}"
            by (rule blockR_pairwise_disjoint[OF j'R jR ne])
          from i_in disj0 have "i \<notin> blockR_abs enc0 as s kk j" by auto
        ...
        next
          ...
          have "map ((!) y) [a'..<a' + w'] ! t = y ! (a' + t)" 
            using idx by (simp add: tw)
          also have "... = ?x ! (a' + t)" using agree_on_j' mem by simp
        ...
        show "Rval_at as s ((!) y) j' = Rval_at as s ((!) ?x) j'"
          using Rval_at_def a'_def w'_def slice_eq_j' by presburger
      qed
      ...
      have good_unread_eq:
        ...
        using Good_unread_R[OF not_touch, of as s kk] outside_y blk_repr by blast
    ...
    next
      ...
      have inside_y:
        "\<And>i. i \<in> {a..<a + w} \<Longrightarrow> y ! i = oR' i"
      proof -
        fix i assume "i \<in> {a..<a + w}"
        then obtain t where t: "t < w" and i_eq: "i = a + t"
          by (auto simp: atLeastLessThan_iff)
        then have ai: "a \<le> i" and ilt: "i < a + w" by simp_all
        have "y ! i = (map oR' [a..<a + w]) ! (i - a)"
          by (simp add: y_def splice_nth_inside[OF LEN BND ai ilt])
        also have "... = oR' ([a..<a + w] ! (i - a))" using t by simp
        also have "... = oR' i" by (simp add: i_eq t)
        finally show "y ! i = oR' i" .
      qed
      ...
      proof -
        have "Rval_at as s ((!) y) j = from_bits (map ((!) y) [a..<a + w])"
          by (simp add: Rval_at_def a_def w_def)
        also have "... = from_bits (map oR' [a..<a + w])"
          by (simp add: slice_eq)
        also have "... = from_bits pat"
          by (simp add: slice_pat)
        also have "... = v_in" by (simp add: pat_val)
        finally show ?thesis .
      qed
      ...
      have good_y:
        "Good as s ((!) ?x) ((!) y)"
      proof -
        have "\<exists>jR<length (enumR as s kk). Rval_at as s ((!) y) jR \<in> set (enumL as s kk)"
...
lemma coverage_for_enc_blocks:
  assumes L2: "2 \<le> length (enumL as s kk)"
      ...
      and baseline_only_j:  (* this is your existing L-side assumption *)
        "\<And>j. Good as s ((!) (enc as s kk)) ((!) (enc as s kk)) \<longrightarrow>
             (\<forall>j'<length (enumL as s kk). j' \<noteq> j \<longrightarrow>
                Lval_at as s ((!) (enc as s kk)) j' \<notin> set (enumR as s kk))"
      and hitR:  "\<exists>v\<in>set (enumR as s kk). v \<in> set (enumL as s kk)"
      and missR: "\<exists>v\<in>set (enumR as s kk). v \<notin> set (enumL as s kk)"
      and baseline_only_jR: (* the R-side analogue *)
        "\<And>j. Good as s ((!) (x0 as s)) ((!) (x0 as s)) \<longrightarrow>
             (\<forall>j'<length (enumR as s kk). j' \<noteq> j \<longrightarrow>
                Rval_at as s ((!) (x0 as s)) j' \<notin> set (enumL as s kk))"
  shows
...
proof -
  have L: "\<forall>j<length (enumL as s kk). touches (enc as s kk) (blockL_abs enc0 as s j)"
    by (rule coverage_for_enc_blocks_L[OF L2 hitL missL baseline_only_j])
  have R: "\<forall>j<length (enumR as s kk). touches (enc as s kk) (blockR_abs enc0 as s kk j)"
    by (rule coverage_for_enc_blocks_R[OF R2 hitR missR baseline_only_jR])
  show ?thesis by (simp add: L R)
qed
